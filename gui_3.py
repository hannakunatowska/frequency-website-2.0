...
line_ranges = {
    ...
    1: {"min": 3125, "max": 6250},
    2: {"min": 6250, "max": 9375},
    3: {"min": 9375, "max": 12500},
}

# --- Custom tick positions for each line ---
line_ticks_custom = {
    0: [0, 12.1, 52, 147, 694, 2700, 3125],     # ticks for line 0
    1: [3125, 4200, 5030, 6250],            # ticks for line 1
    2: [6250, 7250, 8185, 9375],            # ticks for line 2
    3: [9375, 10000, 11200, 12500]          # ticks for line 3 ¨
}
...
for b in raw:
    ...
    # Only include bands that intersect 0–12 500 MHz
    if end > start and end >= 0 and start <= 12500:
        # Clip to the 0–12 500 range
        start = max(start, 0)
        end = min(end, 12500)
...
# --- Tkinter setup ---
root = tk.Tk()
root.title("Frequency Allocation 0-12 500 MHz")
